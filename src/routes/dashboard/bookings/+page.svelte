...
<div class="space-y-6">
		...
		<div class="flex flex-wrap items-center gap-2">
			{#each tabs as tab (tab.key)}
				<a
					...
					class="inline-flex min-h-9 cursor-pointer items-center rounded-xl border-2 border-edge px-3 py-1.5 text-xs font-black shadow-[2px_2px_0px_0px_rgb(var(--bento-shadow))] transition-all sm:min-h-0 {activeTab ===
					tab.key
	...
	{:else if data.bookings.rows.length === 0}
		<div class="bento-card bento-card-static space-y-3 py-16 text-center">
			...
			<h3 class="text-base font-black text-ink">{m.bl_empty_title()}</h3>
		...
		<div class="space-y-3">
			{#each data.bookings.rows as booking (booking.id)}
				<a
					...
					class="bento-card bento-card-static block transition-all hover:border-brand-edge"
				>
						...
						<div class="flex min-w-0 items-center gap-3">
							...
							<div class="min-w-0">
								<div class="mb-0.5 flex flex-wrap items-center gap-2">
									<span class="font-mono text-[10px] font-black tracking-wider text-ink-faint">
										{booking.reference}
									</span>
						...
						<div class="flex shrink-0 items-center gap-4">
							<div class="text-right">
								...
								<span class="text-sm font-black text-ink">
									...
									<span class="text-xs text-brand-fg">{booking.currencyCode}</span>
								</span>
								<div class="mt-1">
									<BookingStatusBadge status={booking.escrowStatus} kind="escrow" />
								</div>
							</div>
							<ArrowRight class="h-4 w-4 shrink-0 text-ink-faint" />
						</div>
					</div>
				</a>
			{/each}
