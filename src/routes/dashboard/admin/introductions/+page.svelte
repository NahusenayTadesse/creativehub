<script lang="ts">
	...
	const statusTone: Record<string, string> = {
		...
		contacted: 'border-indigo-500 bg-indigo-100 text-indigo-900',
		connected: 'border-emerald-600 bg-emerald-100 text-emerald-900',
		declined: 'border-red-500 bg-red-100 text-red-900'
	};
</script>
...
<div class="space-y-6">
	...
	{#if data.cases.rows.length === 0}
		<div class="bento-card bento-card-static space-y-3 py-16 text-center">
			...
			<h3 class="text-base font-black text-slate-900">{m.ai_empty_title()}</h3>
			<p class="text-xs font-medium text-slate-600">{m.ai_empty_body()}</p>
		</div>
	{:else}
		<div class="space-y-4">
							...
							<div>
								<div class="mb-1 flex flex-wrap items-center gap-2">
									...
									{#if row.creatorIsClaimed}
										...
										<span
											class="rounded-md border border-emerald-500 bg-emerald-50 px-2 py-0.5 text-[10px] font-black tracking-wider text-emerald-800 uppercase"
										>
											{m.ai_now_claimed()}
										</span>
									{/if}
					...
					<div class="space-y-2 rounded-2xl border-2 border-slate-200 bg-slate-50 p-3">
						<div class="flex flex-wrap items-center justify-between gap-2">
							...
							<span class="text-xs font-black text-slate-900">
								...
								<span class="text-emerald-600">{row.currencyCode}</span>
							</span>
						</div>
						<p class="text-xs font-bold text-slate-800">{row.title}</p>
							...
							<div class="flex flex-wrap justify-end gap-2">
								{#if row.introductionStatus === 'pending'}
									...
									>
										...
										<button
											type="submit"
											class="flex items-center gap-1.5 rounded-xl border-2 border-slate-900 bg-white px-3 py-1.5 text-xs font-black text-slate-900 hover:bg-indigo-50"
										>
